...
def _openai_whisper(audio_path: Path) -> str:
    ...
    try:
        ...
        raise TranscriptionError("openai is not installed but REELNOTES_ASR=openai.") from exc
    if not os.environ.get("OPENAI_API_KEY"):
        raise TranscriptionError("OPENAI_API_KEY is not set but REELNOTES_ASR=openai.")
    client = OpenAI()
